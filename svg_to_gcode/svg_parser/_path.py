...
class Path:
    ...
    __slots__ = "curves", "initial_point", "current_point", "last_control", "canvas_height", "draw_move", \
                "transform_origin", "transformation", "stroke_width"

    def __init__(self, d: str, canvas_height: float, transform_origin=True, transformation=None, stroke_width="0"):
    ...
    def _add_svg_curve(self, command_key: str, command_arguments: List[float]):
        ...
        # Draw straight line
        def absolute_line(x, y, stroke_width=self.stroke_width):
            start = self.current_point
            end = Vector(x, y)

            line = Line(self.transformation.apply_affine_transformation(start),
                        self.transformation.apply_affine_transformation(end), stroke_width=stroke_width)

            self.current_point = end

            return line

        def relative_line(dx, dy, stroke_width=self.stroke_width):
            return absolute_line(*(self.current_point + Vector(dx, dy)), stroke_width)

        def absolute_horizontal_line(x, stroke_width=self.stroke_width):
            return absolute_line(x, self.current_point.y, stroke_width)

        def relative_horizontal_line(dx, stroke_width=self.stroke_width):
            return absolute_horizontal_line(self.current_point.x + dx, stroke_width)

        def absolute_vertical_line(y, stroke_width=self.stroke_width):
            return absolute_line(self.current_point.x, y, stroke_width)

        def relative_vertical_line(dy, stroke_width=self.stroke_width):
            return absolute_vertical_line(self.current_point.y + dy, stroke_width)

        def close_path(stroke_width=self.stroke_width):
            return absolute_line(*self.initial_point, stroke_width)

        # Draw curvy curves
        def absolute_cubic_bazier(control1_x, control1_y, control2_x, control2_y, x, y, stroke_width=self.stroke_width):
            ...
            cubic_bezier = CubicBazier(trans_start, trans_end, trans_control1, trans_control2, stroke_width)
        ...
        def relative_cubic_bazier(dx1, dy1, dx2, dy2, dx, dy, stroke_width=self.stroke_width):
            return absolute_cubic_bazier(self.current_point.x + dx1, self.current_point.y + dy1,
                                         ...
                                         self.current_point.x + dx, self.current_point.y + dy, stroke_width)

        def absolute_cubic_bezier_extension(x2, y2, x, y, stroke_width=self.stroke_width):
            start = self.current_point
            ...
            if self.last_control:
                ...
                bazier = absolute_cubic_bazier(*control1, *control2, *end, stroke_width)
            else:
                bazier = absolute_quadratic_bazier(*control2, *end, stroke_width)

            self.current_point = start

            return bazier

        def relative_cubic_bazier_extension(dx2, dy2, dx, dy, stroke_width=self.stroke_width):
            return absolute_cubic_bezier_extension(self.current_point.x + dx2, self.current_point.y + dy2,
                                                   self.current_point.x + dx, self.current_point.y + dy, stroke_width)

        def absolute_quadratic_bazier(control1_x, control1_y, x, y, stroke_width=self.stroke_width):
            ...
            quadratic_bezier = QuadraticBezier(trans_end, trans_new_end, trans_control1, stroke_width)
        ...
        def relative_quadratic_bazier(dx1, dy1, dx, dy, stroke_width=self.stroke_width):
            return absolute_quadratic_bazier(self.current_point.x + dx1, self.current_point.y + dy1,
                                             self.current_point.x + dx, self.current_point.y + dy, stroke_width)

        def absolute_quadratic_bazier_extension(x, y, stroke_width=self.stroke_width):
            start = self.current_point
            end = Vector(x, y)

            if self.last_control:
                control = 2 * start - self.last_control
                bazier = absolute_quadratic_bazier(*control, *end, stroke_width)
            else:
                bazier = absolute_quadratic_bazier(*start, *end, stroke_width)

            self.current_point = end
            return bazier

        def relative_quadratic_bazier_extension(dx, dy, stroke_width=self.stroke_width):
            return absolute_quadratic_bazier_extension(self.current_point.x + dx, self.current_point.y + dy, stroke_width)
        ...
        def absolute_arc(rx, ry, deg_from_horizontal, large_arc_flag, sweep_flag, x, y, stroke_width=self.stroke_width):
            end = Vector(x, y)
            start = self.current_point
            ...
            arc = EllipticalArc(center, radii, rotation_rad, start_angle, sweep_angle, transformation=self.transformation, stroke_width=stroke_width)

            self.current_point = end
            return arc

        def relative_arc(rx, ry, deg_from_horizontal, large_arc_flag, sweep_flag, dx, dy, stroke_width=self.stroke_width):
            return absolute_arc(rx, ry, deg_from_horizontal, large_arc_flag, sweep_flag, self.current_point.x + dx, self.current_point.y + dy, stroke_width)
